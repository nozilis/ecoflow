...
async def consume_event(routing_key):
    ...
    async with connection:
        ...
        queue = await channel.declare_queue("analytics_transaction_created", durable=True)
            ...
            async for message in queue_iter:
                async with message.process():
                    ...
                    async with async_session_maker() as session: 
                        await handle_transaction_created(data, session)
...
if __name__ == "__main__":
    asyncio.run(consume_event(('transaction.created')))