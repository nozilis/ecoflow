...
@pytest.fixture
def mock_result(fake_user_profile):
    result = MagicMock()
    result.scalar_one_or_none.return_value = fake_user_profile
    return result

@pytest.fixture
def mock_private_result(fake_private_user_profile):
    result = MagicMock()
    result.scalar_one_or_none.return_value = fake_private_user_profile
    return result
...
@pytest.fixture
def mock_owner_result(fake_owner_user_profile):
    result = MagicMock()
    result.scalar_one_or_none.return_value = fake_owner_user_profile
    return result

@pytest.fixture
def mock_username_taken_result(fake_username_taken_user_profile):
    result = MagicMock()
    result.scalar_one_or_none.return_value = fake_username_taken_user_profile
    return result

@pytest.fixture
def mock_email_taken_result(fake_email_taken_user_profile):
    result = MagicMock()
    result.scalar_one_or_none.return_value = fake_email_taken_user_profile
    return result
...
async def test_get_user_profile(get_user_profile_service, mock_db, mock_redis, mock_result, fake_user_profile_id):
...
async def test_get_private_user_profile(get_user_profile_service, mock_db, mock_redis, mock_private_result, fake_private_user_profile_id):
    with pytest.raises(UserProfileNotFound):
        mock_db.execute = AsyncMock(return_value=mock_private_result)
        result = await get_user_profile_service.get_user_profile(fake_private_user_profile_id)
    mock_redis.get.assert_called_once()
    mock_redis.set.assert_not_called()

async def test_user_profile_not_found(get_user_profile_service, mock_db, mock_redis, mock_none_result, fake_not_found_user_profile_id):
    with pytest.raises(UserProfileNotFound):
...
async def test_update_user_profile(mock_publish, get_user_profile_service, mock_db, mock_redis, mock_rabbitmq, mock_owner_result, mock_none_result, fake_user_profile_update_request):
    mock_db.execute = AsyncMock(side_effect=[mock_owner_result, mock_none_result, mock_none_result])
...
async def test_username_conflict_update_user_profile(get_user_profile_service, mock_db, mock_redis, mock_owner_result, mock_username_taken_result, fake_user_profile_update_request):
    with pytest.raises(UsernameConflict):
        mock_db.execute = AsyncMock(side_effect=[mock_owner_result, mock_username_taken_result])
        result = await get_user_profile_service.update_user_profile(fake_user_profile_update_request)
    mock_redis.delete.assert_not_called()

async def test_email_conflict_update_user_profile(get_user_profile_service, mock_db, mock_redis, mock_owner_result, mock_none_result, mock_email_taken_result, fake_email_user_profile_update_request):
    with pytest.raises(EmailConflict):
        mock_db.execute = AsyncMock(side_effect=[mock_owner_result, mock_none_result, mock_email_taken_result])
        result = await get_user_profile_service.update_user_profile(fake_email_user_profile_update_request)
    mock_redis.delete.assert_not_called()

@patch('services.user_profile_core.publish_user_events', new_callable=AsyncMock)
async def test_delete_user(mock_publish, get_user_profile_service, mock_db, mock_redis, mock_rabbitmq, mock_owner_result):
    mock_db.execute = AsyncMock(return_value=mock_owner_result)
